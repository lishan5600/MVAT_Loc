...
def Load_Image_Information(path):
    ...
    image_Root_Dir = r'E:\1. IHC模型\程序subcellular_location\new数据处理\分解图像\图像_分解'
    # 获取图像的路径
...
premodel = torch.load('./new_densnet_0.05_modle40.pt',map_location=torch.device('cpu'))
print(premodel)
## 定义 hook 函数
embeddings = {}
...
premodel.classifier.lin_2.register_forward_hook(get_activation('classifier.lin_2'))
premodel.eval()
train_feature = []
embeddings = {}
...
for inputs, labels, imageName in train_DataLoader:
    ...
    premodel(inputs)
    train_feature.append(copy.deepcopy(embeddings['classifier.lin_2'].cpu().numpy()))
...
for inputs, labels,imageName in val_DataLoader:
    inputs = inputs.to(device)
    premodel(inputs)
    test_feature.append(copy.deepcopy(embeddings['classifier.lin_2'].cpu().numpy()))
    #把图像名，标签，特征整合
...
data_train.to_csv('new_densnet128_train_all.csv')
data_test=pd.DataFrame(test_all)
data_test.to_csv('new_densnet128_test_all.csv')
