...
def write_metrics(gt, predict, path):
    ...
    with open(path, 'w') as f:
        ...
        f.write("Macro f1_socre:            %.4f\n" % score_f1_macro)
        f.write("Micro f1_socre:            %.4f\n" % score_f1_micro)
        f.write("Hamming Loss:              %.4f\n" % h_loss)
        f.write("zero_one_loss:            %.4f\n" % z_o_loss)
        f.write("mAP:                      %.4f\n" % mAP)
        
        #f.write("Mconfusion_matri:            %.4f" % Mconfusion_matri[0,:])
       
        print(Mconfusion_matri)
        #mul_matri=str(Mconfusion_matri)
        #f.write("混淆矩阵:  %.4f\n"  %mul_matri)
        f.write("混淆矩阵: " )
        mul_matri=Mconfusion_matri.reshape(10,-1)
        for i in mul_matri:
             f.write(str(i))
        # return Mconfusion_matri
